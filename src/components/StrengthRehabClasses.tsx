...
const DiscoveryButton: React.FC<{ className?: string; children?: React.ReactNode }> = ({
  ...
  <a
    ...
    className={`inline-flex items-center justify-center px-8 py-4 bg-sky-500 text-white font-bold rounded-lg hover:bg-sky-600 transition-colors duration-200 ${className}`}
  >
    {children}
  </a>
);
...
const SectionHeading: React.FC<{ children: React.ReactNode; className?: string }> = ({
  ...
  <h2 className={`text-2xl md:text-4xl font-bold text-navy-700 mb-6 leading-tight ${className}`}>
    {children}
  </h2>
);
    ...
    <div className="flex flex-col items-center">
      <div className="relative w-full max-w-md mx-auto" style={{ aspectRatio: '1 / 1' }}>
        <svg viewBox="0 0 300 300" className="w-full h-full" aria-hidden="true">
          ...
          {/* Clockwise rotation arrows between stations */}
          {(() => {
            const angleStep = 360 / stations.length;
            const startAngle = -90;
    ...
    <div className="flex flex-col md:flex-row items-stretch gap-2 md:gap-0">
      {progressionStages.map((stage, i) => (
        <React.Fragment key={i}>
          <div className="flex-1 bg-white rounded-xl shadow-sm border border-navy-100 p-5 text-center md:mx-1">
            <div className="inline-flex items-center justify-center w-10 h-10 bg-sky-100 rounded-full mb-3">
          ...
          {i < progressionStages.length - 1 && (
            <div className="flex items-center justify-center md:px-0">
              <ArrowRight size={20} className="text-sky-400 rotate-90 md:rotate-0" />
            </div>
          )}
        </React.Fragment>
      ))}
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="py-16 md:py-20 bg-white">
        <div className="container mx-auto px-4">
          <div className="grid md:grid-cols-2 gap-10 md:gap-16 items-center max-w-6xl mx-auto">
            ...
            <div>
              <div className="bg-navy-50 rounded-2xl p-8 md:p-10 border border-navy-100">
                <blockquote className="text-xl md:text-2xl font-semibold text-navy-700 leading-relaxed">
              ...
              {/* Image placeholder — authentic photo can be added here later */}
              <div className="mt-6 rounded-2xl border-2 border-dashed border-navy-100 bg-gray-50 flex items-center justify-center text-gray-400 text-sm py-16">
      ...
      <section className="py-16 md:py-20 bg-gray-50">
        <div className="container mx-auto px-4 max-w-6xl">
          <SectionHeading className="text-center mb-4">Who are the classes for?</SectionHeading>
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            {audienceCards.map((card, i) => (
              <div key={i} className="bg-white rounded-xl shadow-sm border border-gray-100 p-7">
                <h3 className="font-bold text-navy-700 text-lg mb-3">{card.title}</h3>
                <p className="text-gray-600 leading-relaxed">{card.body}</p>
              </div>
            ))}
          </div>
          <p className="text-gray-500 text-sm mt-8 max-w-3xl mx-auto text-center leading-relaxed">
      ...
      <section className="py-16 md:py-20 bg-white">
        <div className="container mx-auto px-4 max-w-6xl">
          <SectionHeading className="text-center mb-10">What happens during a class?</SectionHeading>
          ...
          <div className="grid md:grid-cols-2 gap-10 md:gap-16 items-center">
            <div>
              ...
              <div className="bg-white rounded-xl p-6 border border-navy-100">
                <blockquote className="text-lg font-semibold text-navy-700 leading-relaxed">
      ...
      <section className="py-16 md:py-20 bg-white">
        <div className="container mx-auto px-4 max-w-6xl">
          <SectionHeading className="text-center mb-4">How you build strength</SectionHeading>
          <p className="text-gray-600 text-center max-w-2xl mx-auto mb-12 leading-relaxed">
          ...
          <div className="mt-12 max-w-3xl mx-auto">
            <div className="bg-sky-50 rounded-xl p-6 md:p-8 border border-sky-100">
              <h3 className="font-bold text-navy-700 text-lg mb-3">Joining part-way through a block</h3>
      ...
      <section className="py-16 md:py-20 bg-gray-50">
        <div className="container mx-auto px-4 max-w-4xl">
          <SectionHeading className="text-center mb-4">Current classes</SectionHeading>
          <p className="text-gray-600 text-center max-w-2xl mx-auto mb-10 leading-relaxed">
            Maximum five participants per class. You can choose whichever day and time suits you — the Monday and Wednesday classes follow the same overall programme structure, so you don&rsquo;t need to attend a particular day.
          </p>

          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="grid grid-cols-3 bg-navy-700 text-white text-sm font-semibold">
              <div className="px-5 py-4 flex items-center gap-2"><Calendar size={16} /> Day</div>
              <div className="px-5 py-4 flex items-center gap-2"><Clock size={16} /> Time</div>
              <div className="px-5 py-4 flex items-center gap-2"><Users size={16} /> Availability</div>
            </div>
            {classTimetable.map((slot, i) => (
              <div key={i} className={`grid grid-cols-3 border-t border-gray-100 ${i % 2 === 1 ? 'bg-gray-50/60' : ''}`}>
                <div className="px-5 py-4 text-gray-700 font-medium">{slot.day}</div>
                <div className="px-5 py-4 text-gray-600">{slot.time}</div>
                <div className="px-5 py-4">
                  {slot.spaces === 0 ? (
                    <span className="inline-flex items-center px-3 py-1 rounded-full bg-gray-100 text-gray-500 font-medium text-sm">Fully booked</span>
                  ) : (
                    <span className={`inline-flex items-center px-3 py-1 rounded-full font-medium text-sm ${slot.spaces === 1 ? 'bg-amber-50 text-amber-700' : 'bg-sky-50 text-sky-700'}`}>
      ...
      <section className="py-16 md:py-20 bg-white">
        <div className="container mx-auto px-4 max-w-4xl">
          <SectionHeading className="text-center mb-10">Membership &amp; pricing</SectionHeading>
          <div className="bg-navy-700 rounded-2xl p-8 md:p-12 text-center text-white">
            <p className="text-sm text-sky-300 font-semibold uppercase tracking-wide mb-2">By Direct Debit</p>
            <p className="text-5xl font-bold mb-2">&pound;65<span className="text-2xl font-medium text-gray-300"> / month</span></p>
            <p className="text-gray-300 max-w-xl mx-auto leading-relaxed mt-4">
              Your membership reserves your place in your chosen weekly class. This works out at around &pound;15 per class on average across the year.
            </p>
          </div>
          ...
          <div className="grid sm:grid-cols-3 gap-4 mt-8">
            {[
              ...
              'If we have to cancel a class, we will credit the &pound;15 average session value',
            ].map((item, i) => (
          ...
          <p className="text-xs text-gray-400 text-center mt-6 max-w-2xl mx-auto">
            The &pound;15 figure is an average across the year, not the exact cost of an individual class.
          </p>
        </div>
      </section>
      ...
      <section className="py-16 md:py-20 bg-gray-50">
        <div className="container mx-auto px-4 max-w-5xl">
          <SectionHeading className="text-center mb-12">How to get started</SectionHeading>
          <div className="grid md:grid-cols-3 gap-8">
            {gettingStarted.map((step, i) => (
              <div key={i} className="bg-white rounded-xl shadow-sm border border-gray-100 p-7">
                <div className="text-3xl font-bold text-sky-300 mb-3">{step.num}</div>
                <h3 className="font-bold text-navy-700 text-lg mb-3">{step.title}</h3>
      ...
      {/* ---- 11. FINAL CTA ---- */}
      <section className="py-14 md:py-20 bg-navy-800 text-white">
        <div className="container mx-auto text-center px-4 max-w-2xl">
          <h2 className="text-2xl md:text-4xl font-bold mb-4 leading-tight">Ready to get stronger?</h2>
